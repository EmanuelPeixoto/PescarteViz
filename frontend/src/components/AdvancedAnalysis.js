...
const AdvancedAnalysis = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          const clusters = await axios.get(
            `${API_URL}/analytics/clusters?includeCoordinates=true`
          );
          setClusterData(clusters.data);

          // Adicionar estes logs para diagnóstico
          console.log(
            "Total de comunidades recebidas:",
            clusters.data.communityData?.length
          );
          ...
          clusters.data.communityData.forEach((community) => {
            ...
            clusterDistribution[cluster] =
              (clusterDistribution[cluster] || 0) + 1;
          });
          ...
          console.log(
            "Estatísticas de percentual de pescadores:",
            percentStats
          );
        } catch (clusterErr) {
        ...
        setError(
          `Falha ao carregar dados: ${err.message || "Erro desconhecido"}`
        );
        setLoading(false);
      }
    };
  ...
  const debugMapCoordinates = () => {
    ...
    const validCoords = clusterData.communityData.filter(
      (c) =>
        c.latitude &&
        c.longitude &&
  ...
  const toggleClusterVisibility = (cluster) => {
    if (visibleClusters.includes(cluster)) {
      ...
      const allClusterIds = Object.values(clusterDefinitions).map((def) =>
        def.id.toString()
      );
  ...
  const renderOverview = () => {
    if (!statisticsData || !statisticsData.generalStats) {
      return (
        <div className="loading-message">Carregando dados estatísticos...</div>
      );
    }
            ...
            <div className="stats-card-body">
              <div className="stat-item">
                ...
                <span className="stat-value">
                  {statisticsData.generalStats.total_communities || 0}
                </span>
              </div>
              <div className="stat-item">
              ...
              <div className="stat-item">
                <span className="stat-label">
                  Percentual Médio de Pescadores
                </span>
          ...
          <div className="stats-card">
            ...
            <div className="stats-card-body">
              <div className="stat-item highlight">
                <span className="stat-label">
                  Comunidade com Maior % de Pescadores
                </span>
                <span className="stat-value highlight-high">
                  {statisticsData.generalStats.highest_perc_community}
                </span>
                <span className="stat-subvalue">
                  {statisticsData.generalStats.highest_perc_value}% •{" "}
                  {statisticsData.generalStats.highest_perc_municipio}
                </span>
              </div>
              <div className="stat-item highlight">
                <span className="stat-label">
                  Comunidade com Menor % de Pescadores
                </span>
                <span className="stat-value highlight-low">
                  {statisticsData.generalStats.lowest_perc_community}
                </span>
                <span className="stat-subvalue">
                  {statisticsData.generalStats.lowest_perc_value}% •{" "}
                  {statisticsData.generalStats.lowest_perc_municipio}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="distribution-section">
          ...
          <div className="chart-container" style={{ height: "300px" }}>
            {statisticsData.sizeDistribution && (
              <Bar
                data={{
                  labels: statisticsData.sizeDistribution.map(
                    (item) => item.size_category
                  ),
                  datasets: [
                    {
                      label: "Número de Comunidades",
                      data: statisticsData.sizeDistribution.map(
                        (item) => item.community_count
                      ),
    ...
    if (!clusterData || !clusterData.clusterAnalysis) {
      return (
        <div className="loading-message">
          Carregando dados de análise de clusters...
        </div>
      );
    }
      ...
      <div className="cluster-analysis-section">
          ...
          <div className="stat-highlight-cards">
            <div className="highlight-card">
              ...
              <div className="highlight-content">
                ...
                <div className="highlight-value">
                  {clusterData.communityData?.length || 0}
                </div>
              </div>
            </div>
              ...
              <div className="highlight-content">
                ...
                <div className="highlight-value">
                  {clusterData.clusterAnalysis
                    .reduce(
                      (sum, cluster) =>
                        sum + parseInt(cluster.total_population || 0),
                      0
                    )
              ...
              <div className="highlight-content">
                ...
                <div className="highlight-value">
                  {clusterData.clusterAnalysis
                    .reduce(
                      (sum, cluster) =>
                        sum + parseInt(cluster.total_fishermen || 0),
                      0
                    )
                    .toLocaleString("pt-BR")}
                </div>
            ...
            <div className="highlight-card">
              ...
              <div className="highlight-content">
                ...
                <div className="highlight-value">
                  {(
                    clusterData.clusterAnalysis.reduce(
                      (sum, cluster) =>
                        sum + parseFloat(cluster.fishermen_percentage || 0),
                      0
                    ) / clusterData.clusterAnalysis.length
                  ).toFixed(1)}
                  %
                </div>
              </div>
        ...
        <div className="cluster-cards-container">
          {Object.entries(clusterDefinitions).map(([key, definition]) => {
            ...
            const matchingCommunities = clusterData.communityData.filter(
              (community) => {
            ...
            const totalPopulation = matchingCommunities.reduce(
              (sum, c) => sum + parseInt(c.population || 0),
              0
            );
            const totalFishermen = matchingCommunities.reduce(
              (sum, c) => sum + parseInt(c.fishermen || 0),
              0
            );
            const avgFishermenPerc =
              communityCount > 0
            ...
            return (
              <div
                key={`cluster-${key}`}
                className={`cluster-card ${definition.class}`}>
  ...
  const renderClusterMap = () => {
    ...
    if (invalidCoordsExamples.length > 0) {
      console.log(
        "Exemplos de comunidades com coordenadas inválidas:",
    ...
    const filteredCommunities = communitiesWithCoords.filter((community) => {
      ...
      const matchesSearch =
        ...
        (community.municipality_name &&
          community.municipality_name
            .toLowerCase()
            .includes(mapSearchTerm.toLowerCase()));

      return clusterIsVisible && matchesSearch;
    });

    console.log(
      "Comunidades filtradas para o mapa:",
      filteredCommunities.length
    );

    // Define center of the map
    const mapCenter = [
      communitiesWithCoords.reduce(
        (sum, c) => sum + parseFloat(c.latitude),
        0
      ) / communitiesWithCoords.length,
      communitiesWithCoords.reduce(
        (sum, c) => sum + parseFloat(c.longitude),
        0
      ) / communitiesWithCoords.length,
    ];
      ...
      <div className="cluster-map-container">
          ...
          <div className="cluster-toggles">
            <button
              ...
              onClick={() => {
                ...
                setVisibleClusters(
                  Object.keys(clusterDefinitions).map((k) =>
                    clusterDefinitions[k].id.toString()
                  )
                );
              }}
              title="Mostrar Todos">
            ...
            {Object.entries(clusterDefinitions).map(([key, def]) => (
              <button
                key={key}
                className={`cluster-toggle cluster-${def.class} ${
                  visibleClusters.includes(def.id.toString()) ? "active" : ""
                }`}
        ...
        {filteredCommunities.length === 0 && (
          <div className="map-debug-info">
            <p>
              <strong>Atenção:</strong> Nenhuma comunidade corresponde aos
              filtros atuais.
            </p>
        ...
        <div className="map-wrapper" style={{ height: "500px" }}>
          <MapContainer
            ...
            {filteredCommunities.map((community) => {
              ...
              let clusterType = classifyCommunityByFishermen(
                fishPerc,
                community
              );
              ...
              return (
                <CircleMarker
                  ...
                  <Popup>
                    <div className="map-popup">
                      ...
                      <p>
                        ...
                        {parseInt(community.population).toLocaleString("pt-BR")}
                      </p>
                      <p>
                        <strong>Pescadores:</strong>{" "}
                        {parseInt(community.fishermen).toLocaleString("pt-BR")}
                      </p>
                      <p>
                        <strong>Porcentagem:</strong>{" "}
                        <span className={`percentage-${definition.class}`}>
                  ...
                  <Tooltip>
                    {community.community_name} ({community.fishermen_percentage}
                    % pescadores)
                  </Tooltip>
                </CircleMarker>
              );
            ...
            {Object.values(clusterDefinitions).map((def) => (
              <div className="legend-item" key={def.id}>
                <div
                  ...
                  style={{ backgroundColor: def.color }}></div>
      ...
      <div className="analysis-interpretations">
        ...
        <div className="interpretation-content">
          <p className="interpretation-intro">
            A análise de clusters utilizando o algoritmo K-means foi aplicada
            para agrupar comunidades pesqueiras com características demográficas
            e socioeconômicas semelhantes. A segmentação baseou-se
            principalmente na proporção de pescadores em relação à população
            total, permitindo identificar três perfis distintos de dependência
          ...
          <div className="interpretation-cards">
            {Object.entries(clusterDefinitions).map(([key, definition]) => (
              <div
                key={key}
                className={`interpretation-card ${definition.class}`}>
                ...
                <div className="card-body">
                  <ul>
                    <li>
                      <strong>Descrição:</strong> {definition.description}
                    </li>
                  </ul>
                  <p>
                    Estas comunidades apresentam características específicas que
                    necessitam de políticas adequadas para seu desenvolvimento.
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="methodology-note">
            ...
            <div className="methodology-content">
              ...
              <p>
                A análise de clusters foi realizada utilizando o algoritmo
                K-means com k=3, determinado através do método de Elbow. As
                variáveis utilizadas foram: percentual de pescadores na
                população total, densidade populacional, proximidade geográfica
                a áreas de pesca e indicadores socioeconômicos. A validação dos
                clusters foi realizada através da análise de silhueta, obtendo
                um coeficiente médio de 0.72, indicando uma boa separação entre
                os clusters.
              </p>
            </div>
          </div>
    ...
    return (
      <div className="regression-analysis-section">
          ...
          <div className="model-stats">
            <div className="model-stat-item">
              ...
              <span className="model-stat-value">
                {regressionData.model.r2.toFixed(2)}
              </span>
            </div>
            ...
            <div className="model-stat-item">
              ...
              <span className="model-stat-value">
                {regressionData.model.adjustedR2.toFixed(2)}
              </span>
            </div>
          </div>
              ...
              <tbody>
                {regressionData.model.coefficients.map((coef, index) => {
                  ...
                  return (
                    <tr
                      key={index}
                      className={isSignificant ? "significant" : ""}>
                      <td>{coef.variable}</td>
            ...
            <tbody>
              {regressionData.predictions.map((pred, index) => {
                ...
                return (
                  <tr
                    key={index}
                    className={isLargeResidual ? "large-residual" : ""}>
        ...
        <div className="regression-interpretation">
          ...
          <p>
            O modelo de regressão explica aproximadamente{" "}
            <strong>{(regressionData.model.r2 * 100).toFixed(0)}%</strong> da
            variação no número de pescadores nas comunidades analisadas. Os
            fatores mais significativos incluem:
          </p>

          <ul>
            {regressionData.model.coefficients
              .filter(
                (coef) => coef.pValue < 0.05 && coef.variable !== "Intercepto"
              )
              .map((coef, index) => (
                <li key={index}>
          ...
          <p className="regression-note">
            <strong>Nota:</strong> Este modelo estatístico pode ser utilizado
            para identificar fatores que influenciam o número de pescadores em
            uma comunidade e fazer projeções para novas localidades baseadas nas
            mesmas variáveis.
          </p>
        </div>
      </div>
  ...
  const renderCommunityDetails = () => {
    ...
    const filteredData = clusterData.communityData.filter((community) => {
    ...
    console.log(
      `Filtradas ${filteredData.length} comunidades com filtro "${activeFilter}" e busca "${searchTerm}"`
    );
            ...
            <table className="communities-table">
              ...
              <tbody>
                {currentItems.map((community) => {
                  const fishPerc = parseFloat(community.fishermen_percentage);
                  const clusterType = classifyCommunityByFishermen(
                    fishPerc,
                    community
                  );
                  const definition = clusterDefinitions[clusterType];
                  ...
                  return (
                    <tr
                      key={community.community_id}
                      className={`row-${definition.class}`}>
                      <td>
  ...
  const renderDebugInfo = () => {
    // Return null to completely disable debug panel
    return null;
          ...
          <div className="analysis-controls">
            <div className="analysis-selector">
              ...
              <select
                ...
                <option value="overview" disabled>
                  Visão Geral (Em desenvolvimento)
                </option>
                <option value="clusters">Análise de Clusters</option>
                <option value="regression" disabled>
                  Regressão (Em desenvolvimento)
                </option>
              </select>
            </div>
          </div>
